...
import { warning, defaultTransformer, isRedirect, isNotFound } from '@tanstack/react-router';
...
function c(e, t) {
  const r = t || e || {};
  return typeof r.method > "u" && (r.method = "GET"), { options: r, middleware: (n) => c(undefined, Object.assign(r, { middleware: n })), validator: (n) => c(undefined, Object.assign(r, { validator: n })), handler: (...n) => {
    const [a, s] = n;
    ...
    return Object.assign(async (o) => h(i, "client", { ...a, method: r.method, data: o == null ? undefined : o.data, headers: o == null ? undefined : o.headers, context: {} }).then((d) => {
      ...
      return await h(i, "server", { ...a, ...d }).then((u) => ({ result: u.result, error: u.error, context: u.sendContext }));
    } });
  } };
}
function O(e) {
...
const f = async (e, t, r) => e({ ...t, next: async (n = {}) => {
  var _a, _b;
  return r({ ...t, ...n, context: { ...t.context, ...n.context }, sendContext: { ...t.sendContext, ...(_a = n.sendContext) != null ? _a : {} }, headers: T(t.headers, n.headers), result: n.result !== undefined ? n.result : t.result, error: (_b = n.error) != null ? _b : t.error });
} });
function M(e, t) {
  if (e == null) return {};
  if ("~standard" in e) {
...
async function h(e, t, r) {
  const n = H([...b, ...e]), a = async (s) => {
    const i = n.shift();
    if (!i) return s;
    i.options.validator && (t !== "client" || i.options.validateClient) && (s.data = await M(i.options.validator, s.data));
...
function R(e) {
  return E$1(v, "\u{1F6A8}A process.env.TSS_SERVER_FN_BASE env variable is required for the server functions ssr runtime, but was not provided."), `/${j(v)}/${e}`;
}
const p = (e, t) => {
  const r = R(e);
...
async function y() {
  return parseInt(await w.promises.readFile(_, "utf-8").catch(() => "0"));
}
const N = p("app_routes_index_tsx--getCount_createServerFn_handler", (e) => B.__executeServer(e)), B = c({ method: "GET" }).handler(N, () => y()), $ = p("app_routes_index_tsx--updateCount_createServerFn_handler", (e) => k.__executeServer(e)), k = c({ method: "POST" }).validator((e) => e).handler($, async ({ data: e }) => {
  const t = await y();
  await w.promises.writeFile(_, `${t + e}`);
});

export { N as getCount_createServerFn_handler, $ as updateCount_createServerFn_handler };
//# sourceMappingURL=index-CwdCRWfl.mjs.map
